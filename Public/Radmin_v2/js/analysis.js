...
var data1=new Array(), data2=new Array(), data3=new Array(), data4=new Array(), data5=new Array(),data6= new Array();
...
$(document).ready(function() {
	var money = $('.money').children('span').text().replace(/(?=\B(?:\d{3})+\b)(\d{3}(?:\.\d+$)?)/g,',$1');
	$('.money').children('span').text(money);
	//	$(function(){
	//		$.ajax({
	//			type:"get",
	//			url:"",
	...
	var myoption1 = option = {
		...
		legend: {
			...
			formatter: function(name) {
				return echarts.format.truncateText(name, 30, '14px Microsoft Yahei', '…');
			},
			tooltip: {
				show: true,
			},
...
//将后台加载到的数据传到地图,并显示出来
function loadArea(mapDetail, param) {
	if(mapDetail.iscity != true) {
	...
	getDataMap(mapDetail.urls, mapDetail.mapname, mapDetail.mapdata);
}

//返回的方法
function backArea(mapDetail, param) {
	if(mapDetail.iscity == true) {
		//初始化
...
function getlevel(aim){
	...
	function(data){
		...
		for(var i=0;i<data.count.length;i++){
			data4.push({name:data.name[i],value:Number(data.count[i])})
			data6.push({name: data.name[i],textStyle: {color: '#fff'}})
		}
		data4 = eval(data4);
		data6 = eval(data6);
		aim.setOption({
...
function getkind(aim,months){
	...
	function(data){
		...
		$.each(data.info.list, function(key,val) {
			data5.push({name:val.templet.name,value:Number(val.cost_num)})
			data6.push({name: val.templet.name,textStyle: {color: '#fff'}})
		});
		data5=eval(data5);
		data6=eval(data6);
		aim.setOption({
...
function getSalecount(aim,months){
	...
	function(data){
		...
		console.log(data)
		$.each(data.info.list,function(key,val){
			data3.push({name:val.dis_info.name,value:Number(val.buy_money),itemStyle: {normal: {color: bgcolor[key]}}})
			data6.push(val.dis_info.name)
		});
		if(data6.length<2){
			data3.push({name:"暂无数据",value:'',itemStyle: {normal: {color: bgcolor[2]}}})
			data6.push("暂无数据");
		}
		console.log(data3)
