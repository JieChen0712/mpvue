$(document).ready(function() {
    ...
    $('#submit-order').click(function() {
        ...
        p_nums = num.split("|");
        if($('#flimg').val() == "" && !(getUrlParam('order') == undefined || getUrlParam('order') == null || getUrlParam('order') == "")) {
            tusi("请上传支付截图");
            return false;
        } else {
            pay_photo = $('#flimg').val();
        }
        if(shipping_open && shipping_way_id != undefined) {
        ...
        var shipping_way_id = $('input[name=shipping_way_id]').val().toString();
        ...
        $.post("orderhand", {
            ...
            city: $('#city').val(),
            county: $('#county').val(),
            s_name: $('#re_name').val().trim(),
            s_phone: $('#re_phone').val().trim(),
            auto_order_num: getUrlParam('order'),
            addre: $('#address').text().trim(),
            pay_photo: pay_photo,
            note: note,
            sku_ids: sku_ids,
            ...
            if(data.code == 1) {
                ...
                //        				}
                setTimeout(function() {
                    window.location.href = now_url + "examine";
                }, 1500);
    ...
    //  选择运费方式
    //$("#picker-way").picker({
    //  toolbarTemplate: '<header class="bar bar-nav"><button class="button button-link pull-left">按钮</button>' +
    //    '<button class="button button-link pull-right close-picker choose-way">确定</button><h1 class="title">标题</h1></header>',
    //  cols: [{
    //    textAlign: 'center',
    //    values: selectArr
    //  }]
