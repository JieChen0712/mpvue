$().ready(function() {
    ...
    switch (part) {
        ...
        case '6':
            $('.buttons-tab a').eq(2).addClass('active');
            target = $("#tab6").children(".content-block");
            $('#tab6').addClass('active');
            stat = 6;
            break;
        case '8':
            $('.buttons-tab a').eq(2).addClass('active');
            target = $("#tab8").children(".content-block");
                ...
                if(data.code == 1) {
                    ...
                    $.each(data.info.list, function(key, value) {
                        ...
                        for(var i = 0; i < value.length; i++) {
                            ...
                            goodsImg = value[i].templet.image;
                            userName = value[i].u_name;
                            orderStatus = value[i].status_name;
                            total_num = value[i].total_num;
                            shipping_fee = value[i].shipping_fee;
                            var is_shipping = true;
                            if(shipping_fee==undefined||shipping_fee==null||shipping_fee==""){
                                is_shipping = false;
                                total_price = Number(value[i].total_price).toFixed(2);
                            }else{
                                total_price = (Number(value[i].total_price) + Number(shipping_fee)).toFixed(2);
                            }
                            
                            if(stock==undefined||stock==null||stock==""){
                                if(value[i].status == 1) {
                                    btnHtml = '<div class="order-btn"><input type="button" value="取消订单" data-id="' + key + '" class="button button-dark cancel-order"></div></dl>';
                                ...
                                else if(value[i].status == 8) {
                                    btnHtml = '<div class="order-btn"><input type="button" value="抢单" class="button button-dark order-grab" data-id="' + key + '"></div></dl>';
                                }
                            }
                        ...
                        var str2 = '<p class="goods-sum"><span>共' + total_num + '件商品</span><span>合计:￥' + total_price + (is_shipping?'(含运费:￥' + shipping_fee + ')</span></p></dd>':'');
