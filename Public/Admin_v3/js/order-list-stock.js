$(document).ready(function() {
    ...
    $('#submit-order1').click(function() {
        ...
        p_ids = $('input[name=product_id]').val();
        p_nums = $('input[name=shownum]').val();
        var cart_ids = $('input[name=cart_ids]').val();
        var pay_photo = $('#flimg').val();
        var pay_type = Number($('input[name=payWay]:checked').val());
        if(pay_type == '') {
            tusi('请选择支付方式！');
        ...
        //买货方式
        var buy_way = $("input[name='buy_way']:checked").val();
        ...
        $.post("orderstock", {
            ...
            $(that).removeAttr('disabled');
            //让按钮恢复点击
            $('#submit-order1').removeAttr('disabled');
        });
    });
