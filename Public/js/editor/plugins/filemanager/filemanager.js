		...
		function createView(result) {
			...
			(function($){
				...
				$(".xl_span").click(function(){ //这里的.xl_span对应着刚才插入的删除按钮上的class
				    ...
				    $.post('../../Public/Js/editor/plugins/filemanager/file_manager_json.php',{action:"delete",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
				        res==1?$this.parent().remove():alert(res);//如果返回1则直接删除 图片，名字的Div达到即时删除，否则提示
