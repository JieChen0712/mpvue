...
//		$this->trigger(self::EVENT_AFTER_SAVE_PROPERTY_VALUES);
    ...
    private function get_prices($templet_id) {
		...
		foreach ($skus as $sku) {
			...
			$prices[$sku['properties']] = $this->get_other_prices($sku, ['price'=>$sku['price'], 'stock'=>$sku['quantity'], 'image' => $sku['image']]);
			$price = $sku['price'];
    ...
    public function get_templet_properties($templet_id) {
        ...
        $properties = D('')->field('`tp`.`id`,`tp`.`name`')->table(['templet_to_property'=>'ttp','templet_property'=>'tp'])
        ...
        if (!empty($properties)) {
            $property_values = D('')->field('`tpv`.`id`,`tpv`.`value`,`tpv`.`pid`')
                    ->table(['templet_property_to_value'=>'tptv','templet_property_value'=>'tpv'])
