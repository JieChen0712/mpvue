...
//    private $users;
    ...
    public $order_count_way = 1;//按订单统计
...
//        } else if ($count_way == 1) {
//            $this->money_month_model = M('order_month_count');
//        }
        ...
        $this->team_setting = $this->rebate_team_setting_model->where($map)->order('achievement asc')->select();
    }

    //生成团队奖
    public function create_team_rebate($id, $month, $users) {
        if ($month) {
            $this->month = $month;
        }
        if ($users) {
...
//            setLog(json_encode($users));
        ...
        foreach ($users as $user) {
            if ($this->is_parent_audit && $this->team_setting[0]['achievement_way'] == $this->money_count_way) {
                //如果按虚拟币统计，并且是上级审核，则只统计自己充值的虚拟币
                $uids = $user['id'];
            } else {
                //获取团队信息
            ...
            //团队业绩
            if($this->team_setting[0]['achievement_way'] == $this->order_count_way){
                $where['day'] = isset($where['day'])?$where['day']:0;
                $where['pid']   =   0;
                $team_money = $this->order_count_model->where($where)->sum('buy_money');
            }
            else{
                $team_money = $this->money_month_model->where($where)->sum('money');
            }
...
//            }
            ...
            $ratio = $this->get_team_rebate_ratio($team_money);
...
//        $ratio = C('TEAM_REBATE_RATIO');
     ...
     * 团队返利统计
     */
    private function team_rebate_count() {
