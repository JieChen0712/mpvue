...
//-----------****新版经销商树状图结束***---------
    ...
    public function show_team()
    {
        ...
        if ($count > 0) {
            ...
            $p = new Page($count, $page_num);
            $limit = $p->firstRow . "," . $p->listRows;
            $field = 'id,name,wechatnum,phone,levname,audited,time,update_time';
            ...
            foreach ($lower_users as $key => $user) {
                ...
                //团队业绩
                $uids = $team_obj->get_team_ids($user['id'], $team_path);
...
//                'levname' => $levnames[1]
                ...
                $res = $distributor->where(array('id' => $m_id))->setField('audited', 1);
            }
...
//            $this->wechat_obj->sendTemplateMessage($template);
            ...
            if ($recommendID) {
                //查找出推荐人的等级信息
                $rec_info=$distributor->where(['id'=>$recommendID])->find();
                if(($rec_info['level'] == 1 ) && ($signaturea['level'] == 1)){
                    $rebate_result = $Rebate->radmin_user_audit_rebate($m_id, $signaturea);
                }
            }
