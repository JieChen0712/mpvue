...
//        $month=I('month');
        ...
        if( !empty($pid) ){
            ...
            $condition_top['pid']   =   array('gt',0);
        }
        
//        if(!empty($month)){
//            $condition_top['month'] = $month;
//        }
        ...
        if( !empty($s_month) && !empty($en_month) ){
            ...
            elseif ($type == 'day'){
                $condition_top['day'] =   ['between',[$s_month,$en_month]];
            }
        }
...
//        setLog(json_encode($condition_top));
//        if(!$condition_top['month']){
//            $condition_top['month'] =array('gt',0);
//        }

        $count=$order_count_obj->where($condition_top)->order('pid asc')->count('id');
        $page_num=20;
        if($count>0){
            $p = new Page($count, $page_num);
            $limit = $p->firstRow . "," . $p->listRows;
            $top_list = $order_count_obj->where($condition_top)->order('pid asc')->limit($limit)->select();
                foreach( $top_list as $k_top => $v_top ){
                    $v_top_pid = $v_top['pid'];
...
//      var_dump($top_list);die;
        ...
        $list=$order_count_obj->where($boss_condition)->order('cost_money desc')->select();
        foreach( $list as $k => $v ){
            $v_top_pid = $v['pid'];
            $list[$k]['p_name'] = $templet_key_info[$v_top_pid]['name'];
        }
...
//        print_r($this->_post());return;
    ...
    public function order_count_sum(){
        ...
        $count=$order_count_obj->where($boss_condition)->order('cost_money desc')->count('id');
        $count_num=$order_count_obj->where($boss_condition)->sum('cost_num');
        $count_money=$order_count_obj->where($boss_condition)->sum('cost_money');
...
//            $p = new Page($count, $page_num);
//            $limit = $p->firstRow . "," . $p->listRows;
//            $list=$order_count_obj->where($boss_condition)->order('cost_money desc')->limit($limit)->select();
//            foreach( $list as $k => $v ){
//                $v_pid = $v['pid'];
//                $list[$k]['p_name'] = $templet_key_info[$v_pid]['name'];
//            }
