    ...
    public function applyList() {
        ...
        if ($count > 0) {
            ...
            foreach ($applyList as $k => $v) {
                ...
                foreach ($row as $b => $d) {
                    $rol = M('shop_templet')->field('name')->where(array('id' => $d['p_id']))->find();
...
//        print_r(I());return;
    ...
    public function order_count(){
        ...
        $order_count_obj = M('shop_order_count');
...
//        return;
    ...
    public function upload()
    {
        ...
        $upload->savePath = './upload/shoporder/';// 设置附件上传目录
    ...
    public function index_kd(){
        ...
        $info=M('shop_order')->where(array('order_num'=> $order_num))->field('id,order_num,shipper,ordernumber')->find();
        $shipper = AllShipperCode();
        $this->shipper=$shipper;
        $this->info=$info;
        $this->display();
    ...
    public function set_kd(){
        ...
        $res=M('shop_order')->where($condition)->save($data);
        if($res){
