 ...
 *    微斯咖经销商后台——首页
 */
class ShopAction extends TemCommonAction
{
    ...
    {
        parent::_initialize();
        $this->model = M('shop_order');
        $this->templet_model = M('shop_templet');
        $this->templet_cat_model = M('shop_templet_category');
        $this->cart_model = M('shop_order_shopping_cart');
        $this->adv_model = M('shop_advert');
...
//        }
          ...
          if(empty($uid)&&empty($this->uid)){
            exit();
              ...
              $encode_uid = $this->encode_uid();
              $url = __APP__.'/sale/shop/index?uid='.$encode_uid;
              header('Location:'.$url);
//            var_dump($check_share_result);
              exit();
              
          }
              $check_uid_result = $this->check_uid_link($uid);

          if( $check_share_result ){
              return FALSE;
          }
          
          session('shop_uid',$uid);
          $three_day = 60*60*24*3;
          cookie('uid',$uid,array('expire'=>$three_day));
          //首页显示的产品
          $condition_product=[
              'active' => 1,
//            'statu' =>1,
    ...
    //商品详情 new
      public function goods_detail()
      {
          ...
          $id = I('id');
          $shipping_fee = bcadd(C('SHOP_IN_SHOP_SHIPPING'),0,2);
...
//      $this->address = M('mall_address')->where(['user_id' => $this->uid, 'default' => 1])->find();
    ...
    public function buy_cart_detail()
    {
        ...
        $this->address = M('shop_address')->where(['user_id' => $this->uid, 'default' => 1])->find();
        $this->return_url = $this->base_url(__SELF__);
        $this->display();
    }
...
//        $this->display();
    ...
    public function my_dhj()
    {
        ...
        $count = M('shop_templet')->count("id");
        $list = M('shop_templet')->field('id,name,image,disc,state,' . $price)->select();
        foreach ($list as $k => $v) {
            $list[$k]['price'] = $v[$price];
        }
        $manager = $this->manager;
...
//    }
    ...
    public function orderhand()
    {
        ...
        $s_name = I('s_name');
        $s_phone = I('s_phone');
        $s_address = I('s_address');
        $s_address_detail = I('s_address_detail');
        
        $uid = $_SESSION['shop_uid'];
        if(!empty($uid)){
            $uid = $this->decode_uid($uid);
            $uid = $uid['uid'];
            ...
            $write_info = array(
                ...
                's_phone' => $s_phone,
                's_address' => $s_address,
                's_address_detail' => $s_address_detail
            );
...
//        $res = $this->model->where($condition)->save(['status' => 3]);
    ...
    public function ordernb()
    {
        ...
        $save = M('shop_order')->where($condition)->save($update_info);
    ...
    public function all()
    {
        ...
        $this->shipping_fee = $shipping_fee;
        $this->shipping_num = C('SHOP_IN_SHOP_NUM');
        $this->display();
    }
...
//        $type = 'take';
    ...
    public function cancel_order()
    {
        ...
        import('Lib.Action.Shoporder', 'App');
        $order = new Shoporder();
        $result = $order->delorder($order_num,$type);
...
//    }
    ...
    public function goods_kind(){
        ...
        $info_advert=M('shop_advert')->where($condition_one)->order('sequence desc,id desc')->find();
        ...
        //将取出来的id在另外的表根据id查询
        $mats = M('shop_templet_category')->where(['id' => $info_advert['malltemplet_category_id']])->select();
        //取出数据
        foreach ($mats as $v) {
            $material_info['id'] = $v;
        }
    ...
    public function center(){
        //消费额
        $this->business_money = M('shop_order')->where(['user_id' => $this->uid, 'status' => ['egt',2]])->group('order_num')->sum('total_price');
        //佣金
        $this->funds = M('shop_money_funds')->where(['uid' => $this->uid])->find();
        $this->display();
    }
...
//        $condition['recommendID'] = $this->uid;
    ...
    public function goods_all()
    {
        ...
        if( !$check_share_result ){
            $encode_share = $this->encode_share();
            $url = __APP__.'/sale/shop/goods_all?share='.$encode_share.'&name='.$name.'&category_id='.$category_id;
            header('Location:'.$url);
            exit();
        }
...
//        }
    ...
    public function money_refund_apply_submit() {
        ...
        $uid = $this->uid;
        $MALL_REFUND=C('SHOP_REFUND');
        $refund_type=$MALL_REFUND['SHOP_REFUND_PAY_TYPE'];

//        print_r($this->_post());return;

        import('Lib.Action.Shopfunds','App');
        $Funds = new Shopfunds();
...
//        $page_num=1;
        ...
        $result = $Shoporder->get_refund($page_info,$condition);
        $return_result = [
            'code'  =>  1,
            'msg'   =>  '获取成功',
            'info'  =>  $result,
...
//		    $refund_id=4;
		    if($bonus_id != null && $refund_id == null){
		        ...
		        $result = $Shoporder->get_refund([],$condition);
		    }
		    if($bonus_id == null && $refund_id != null){
    ...
    public function success() {
        $this->order = M('shop_order')->where(['order_num' => I('order_num')])->find();
        $this->display();
    }

    //改变购物车产品数量
    public function save_cart_num() {
        $cart_model = M('shop_order_shopping_cart');
        $cart_ids = explode('|', I('cart_ids'));
    ...
    public function del_cart() {
        if (IS_AJAX) {
            $cart_ids = explode('|', I('cart_ids'));
            $res = M('shop_order_shopping_cart')->where(['id' => ['in', $cart_ids]])->delete();
            $this->ajaxReturn($res, 'JSON');
        }
    }
