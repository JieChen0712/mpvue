...
class Wechat
{
	...
	public $logcallback;
        ...
        public $more_wechat = FALSE;//是否多项目应用同一公众号
        public $more_wechat_setcache;//中转接口access_token设置的接口
        public $more_wechat_getcache;//中转接口access_token获取的接口
        
        private $OAUTH_PREFIX;//授权跳转地址，原为常量
        ...
        public function __construct($options)
	{
		...
		$this->logcallback = isset($options['logcallback'])?$options['logcallback']:false;
                ...
                if( $this->more_wechat ){
                    $this->OAUTH_PREFIX = '';
                    $this->more_wechat_setcache = '';
                    $this->more_wechat_getcache = '';
                }
                else{
                    $this->OAUTH_PREFIX = 'https://open.weixin.qq.com/connect/oauth2';
                }
	}
...
// 		setLog($postStr);
	...
	*/
	...
	protected function setCache($cachename,$value,$expired){
            if( !$this->more_wechat ){
                $access_token = $this->setCache_old($cachename,$value,$expired);
            }
            else{
                $access_token = $this->setCache_moresite($cachename,$value,$expired);
            }
	...
	public function getCache($cachename=''){
            if( !$this->more_wechat ){
                $access_token = $this->getCache_old($cachename='');
            }
            else{
                $access_token = $this->getCache_moresite($cachename='');
            }
	 ...
	 * @return boolean
	 */
	protected function setCache_old($cachename,$value,$expired){
...
//                    $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=$this->appId&secret=$this->appSecret";
            ...
            $url = self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appId.'&secret='.$this->appSecret;
	 ...
	 * @return mixed
	 */
	public function getCache_old($cachename=''){
		//TODO: get cache implementation
                $data = json_decode(file_get_contents("access_token.json"));
                if ($data->expire_time < time()) {
                    $access_token = $this->setCache();
                } else {
	 ...
	 * @return boolean
	 */
	protected function setCache_moresite($cachename,$value,$expired){
            $data = json_decode(file_get_contents("access_token.json"));
            $url = self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appId.'&secret='.$this->appSecret;
            $res = json_decode($this->http_get($url));
            $access_token = $res->access_token;
...
//              fclose($fp);
            ...
            $url = $this->more_wechat_setcache;
            $url = $url.'?data='.serialize($data);
            $result = json_decode($this->http_get($url));
	 ...
	 * @return mixed
	 */
	public function getCache_moresite($cachename=''){
	    $url = $this->more_wechat_getcache;
            $result = json_decode($this->http_get($url));
	 ...
	 */
	public function getUserInfo($openid,$return_all=FALSE){
		if (!$this->access_token && !$this->checkAuth()) return false;
	...
	public function getOauthRedirect($callback,$state='',$scope='snsapi_userinfo'){
		return $this->OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL.'appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
	}
