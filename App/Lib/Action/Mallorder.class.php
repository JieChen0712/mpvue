...
class Mallorder {
    ...
    public function __construct() {
        $this->order_obj = M('mall_order');
        $this->distributor_obj = M('distributor');
        $this->templet_obj = M('mall_templet');
    }
...
//        return $order_obj->getLastSql();
    ...
    public function write_order($uid,$write_info){
        ...
        $address = M('mall_address')->where(['user_id' => $uid, 'default' => 1])->find();
...
//            $error_info[]   =   $arr;
        ...
        if( !$addorder ){
            ...
            //如果是购物车就删除id
            if ($cart_ids) {
                M('mall_order_shopping_cart')->where(['id' => ['in', $cart_ids]])->delete();
            }
            return $return_result;
        }
...
//            $order_month = $order_info_key[$order_num]['month'];
            ...
            //----------扣费逻辑------------
            $charge_money_result = $Funds->charge_money($uid,$total_price,'Mallorder',$order_num);
...
//        return $fm_next_month = date("Ymd",$tmp_nextmonth);
    ...
    public function month_count($data) {
        $model = M('mall_order_month_count');
        $month = get_month();
        ...
        if (!$res) {
            setLog('订单月统计失败:'.json_encode($data), 'mall_order_count');
            return false;
        }
        return true;
    }
...
//                $list[$k]['p_info'] = $dis_key_info[$v_pid];
    ...
    public function get_refund($page_info=array(),$condition=array()){
        $mall_money_log = M('mall_money_log');
        $distributor_obj = M('distributor');
        import('ORG.Util.Page');
