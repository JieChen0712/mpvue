...
//            $order_month = $order_info_key[$order_num]['month'];
            ...
            //----------生成一次性返利--------------
            $dis_info=M('distributor')->where(['id' => $order_info_key[$order_num]['user_id']])->find();
            if($type == 'radmin'){
                //edit by qjq  用于meb高推低首次下单订单一次性返利
                $rec_info=M('distributor')->where(['id'=>$dis_info['recommendID']])->find();
                $o_id=$order_info_key[$order_num]['o_id'];
                $condition=[
                    'user_id'=>$uid,
                    'status'=>['gt',1],
                ];
                $first_order=$this->stock_order_model->where($condition)->count('id');
                //如果是向总部下单并且是首单，则触发返利
                if(($rec_info['level'] == 1)&&( $dis_info['level'] == 2 || $dis_info['level'] == 3) && (empty($o_id)) && (empty($first_order))){
                    $Rebate->radmin_user_audit_rebate($uid,$dis_info);
                }
            }
            //----------end 生成一次性返利--------------
