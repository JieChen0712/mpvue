...
class Funds {
    ...
    public $is_charge_money = FALSE;//是否进行虚拟币系统的逻辑，如扣费，下单金额判断

    public $is_charge_money_level = [

    ];//如果有值则根据该值进行级别判断是否进行虚拟币功能逻辑

    public $is_all_can_refund = FALSE;//是否充值金额都可以提现，即可提现金额等于充值金额
    ...
    public $is_parent_audit = TRUE;//是否由直属上级审核虚拟币，是则由上级审核下级充值申请，并上级相应余额转到下级。否则总部审核充值

    public $is_order_return = TRUE;//是否启用订单返还（需要开启扣虚拟币系统is_charge_money）,注意，开启后经销商及总部审核时都会触发
    public $order_return_rank = 100;//订单返还循环的次数

    public $is_rebate_recharge = TRUE;//返利是否充入账户
    ...
    public function __construct() {
        ...
        if( $FUNCTION_MODULE['MONEY'] == '1' ){
            $this->is_charge_money = TRUE;
        }
        
        $extra = C('extra');
        if( isset($extra['funds']) ){
            $extra_info = $extra['funds'];
            
            if( isset($extra_info['is_charge_money_level']) ){
                $this->is_charge_money_level = $extra_info['is_charge_money_level'];
            }
            if( isset($extra_info['is_all_can_refund']) ){
                $this->is_all_can_refund = $extra_info['is_all_can_refund'];
            }
            if( isset($extra_info['is_get_min_apply_money']) ){
                $this->is_get_min_apply_money = $extra_info['is_get_min_apply_money'];
            }
            if( isset($extra_info['is_get_min_refund_money']) ){
                $this->is_get_min_refund_money = $extra_info['is_get_min_refund_money'];
            }
            if( isset($extra_info['is_parent_order']) ){
                $this->is_parent_order = $extra_info['is_parent_order'];
            }
            if( isset($extra_info['is_parent_audit']) ){
                $this->is_parent_audit = $extra_info['is_parent_audit'];
            }
            
            if( isset($extra_info['is_order_return']) ){
                $this->is_order_return = $extra_info['is_order_return'];
            }
            if( isset($extra_info['order_return_rank']) ){
                $this->order_return_rank = $extra_info['order_return_rank'];
            }
            
            if( isset($extra_info['is_rebate_recharge']) ){
                $this->is_rebate_recharge = $extra_info['is_rebate_recharge'];
            }
        }
    }
...
//                $list[$k]['source_info'] = $dis_key_info[$v_source_id];
                ...
                $list[$k]['dis_authnum'] = $dis_key_info[$v_uid]['authnum'];
                $list[$k]['dis_idennum'] = $dis_key_info[$v_uid]['idennum'];
                $list[$k]['dis_time'] = date('Y-m-d H:i',$dis_key_info[$v_uid]['time']);
                $list[$k]['dis_update_time'] = date('Y-m-d H:i',$dis_key_info[$v_uid]['update_time']);
                $list[$k]['dis_headimgurl'] = $dis_key_info[$v_uid]['headimgurl'];
    ...
    public function get_money_charge_log($page_info=array(),$condition=array()){
        ...
        if( $count > 0 ){
            ...
            foreach( $list as $k => $v ){
                ...
                elseif( $v_type == 3 ){
                    $v_order_num_format = '<a href="__URL__/money_apply?id='.$v_order_num.'">申请ID：'.$v_order_num.'</a>';
                }
...
//        $level = $distributor_info['level'];
        ...
        //查看该代理的资金表
        $field = 'recharge_money,can_refund_money';
        ...
        $recharge_money = $money_funds['recharge_money'];
        $can_refund_money = $money_funds['can_refund_money'];
...
//        $can_refund_money = $this->get_can_refund_money($level,$recharge_money);
    ...
    public function check_recharge_money($uid,$money,$check_order=FALSE){
        ...
        if( !empty($this->is_charge_money_level) ){
            $user = M('distributor')->where(['id'=>$uid])->field('level')->find();
            $user_level = isset($user['level'])?$user['level']:0;
            if( !in_array($user_level, $this->is_charge_money_level) ){
                $return_restult = array(
                    'code'  =>  1,
                    'msg'   =>  '经销商级别不可用虚拟币系统！',
                );
...
//        $distributor_obj = M('distributor');
        ...
        $recharge_money = $funds_info['recharge_money'];//当前可用金额
     ...
     * @return int
     */
    public function add_money_apply($uid,$audit_id,$apply_money,$apply_img='',$pay_way){
...
//                ),
     ...
     * @return int
     */
    public function add_money_refund_apply($uid,$audit_id,$apply_money,$apply_remark='',$apply_img='',$pay_type,$card_name,$card_number,$account_name){
...
//        $money_funds_obj = M('money_funds');
     ...
     * @return array
     */
    public function apply_refund_pass($id,$pass,$cur_audit_id=0){
        ...
        if( $pass == 1 ){
            ...
            $refund_res = $this->refund($uid,$apply_money,$refund_type);
...
//            $note = $recharge_info['note'];
        ...
        else if( $type == 'refund_stock_recharge' ){
            $apply_id = 0;
            $source_id = 0;
            $recharge_type = 6;//云仓退款
        }
        else{
            $source_id = 0;
            $apply_id = 0;
...
//        if ($this->is_charge_money) {
//            if ( !$this->is_parent_audit && $new_can_refund_money == 0 ) {
//                $this->month_count($recharge_log);
//            } elseif( $this->is_parent_audit ) {
...
//        }
        ...
        $old_funds_his_recharge_money = $funds_info['his_recharge_money'];//历史充值金额
//        $funds_his_charge_money = $funds_info['his_charge_money'];//历史扣费金额
        ...
        $Rebate = new Rebate();
        $is_first = FALSE;//是否首次充值

        if( $old_funds_his_recharge_money == 0 ){
            $is_first = TRUE;
        }

//        $recharge_id = $money_recharge_log_obj->getLastInsID();
        $log = $money_recharge_log_obj->find($recharge_log_result);
        $Rebate->recharge_rebate($uid,$log);
//        $Rebate->recharge_rebate($uid,$money,$recharge_log_result,$is_first);
        //----生成返利---
        ...
        if(C('REBATE')['ORDINARY_TEAM'] && (!C('REBATE')['CLICK_TEAM_REBATE'])){
            $NewRebate->create_team_rebate($uid);
        }
        //------------end触发团队返利结束---------
     ...
     * @return array
     */
    public function refund($uid,$refund_money,$type=1){
...
//        echo bccomp($can_refund_money,$refund_money,2);
        ...
        //原来资金信息里的申请金额
        $old_recharge_money = $funds_info['recharge_money'];
        $old_can_refund_money = $funds_info['can_refund_money'];
        $old_his_refund_money = $funds_info['his_refund_money'];
        ...
        $funds_data = array(
            'recharge_money'    =>  $new_recharge_money,
            'can_refund_money'  =>  $new_can_refund_money,
            'his_refund_money'  =>  $new_his_refund_money,
            'updated'   =>  time(),
        );
    ...
    public function charge_money($uid,$money,$charge_type='order',$order_num=''){
        ...
        $old_his_charge_money = $funds_info['his_charge_money'];
        ...
        $money_funds_save_info = array(
            'recharge_money'    =>  $new_recharge_money,
            'his_charge_money'  =>  $new_his_charge_money,
            'can_refund_money'  =>  $new_can_refund_money,
        );
...
//                ),
    ...
    public function monery_order_return($uid,$order_num,$order_info){
        ...
        if( !$this->is_charge_money || !$this->is_order_return ){
            $return_restult = array(
                'code'  =>  1,
                'msg'   =>  '无须进行订单返还',
            );
        ...
        $recharge_error = array();//充值错误
        $other_error = [];//其它错误
        
        
        //先得到订单相应的tem_id和sku_id
        $sku_ids = [];//产品规格ID
        $tem_ids = [];//产品模板id
        ...
        if( !empty($sku_ids) ){
            ...
            foreach( $new_sku_info as $v ){
                $the_sku_id = $v['id'];
                $sku_info[$the_sku_id] = $v;
            }
        }
        ...
        if( !empty($tem_ids) ){
            ...
            $new_tem_info = $templet_obj->where($condition_tem)->select();
            
            foreach( $new_tem_info as $v_tem ){
                $v_tem_id = $v_tem['id'];
                
                $tem_info[$v_tem_id] = $v_tem;
            }
        }
        ...
        for( $i=1;$i<=$this->order_return_rank;$i++ ){
            ...
            foreach( $order_info as $o_info ){
                ...
                $the_sku_id = $o_info['sku_id'];//产品规格ID
                
                $the_tem_info = isset($tem_info[$the_p_id])?$tem_info[$the_p_id]:[];//产品模板的信息
                $the_sku_info = isset($sku_info[$the_sku_id])?$sku_info[$the_sku_id]:[];//产品规格的信息
                
                $the_old_price = $the_new_price = 0;
                if( !empty($the_sku_id) ){
                    $the_old_price  =   $the_sku_info[$price_key_name];//该等级的下级的产品单价金额
                    $the_new_price  =   $the_sku_info[$new_price_key_name];//该等级的产品单价金额
                }
                elseif( !empty($the_tem_info) ){
                    $the_old_price  =   $the_tem_info[$price_key_name];//该等级的下级的产品单价金额
                    $the_new_price  =   $the_tem_info[$new_price_key_name];//该等级的产品单价金额
                }
                elseif( empty($the_sku_info) && empty($the_tem_info) ){
                    $other_error[] = [
                        'msg'   =>  '产品模板和产品规格都没有信息',
                        'the_tem_info'  =>  $the_tem_info,
                        'the_sku_info'  =>  $the_sku_info,
                        'tem_info'  =>  $tem_info,
                        'sku_info'  =>  $sku_info,
                        'sku_ids'   =>  $sku_ids,
                        'tem_ids'   =>  $tem_ids,
                ...
                //一般而言，价格不可能为0
                if( empty($the_old_price) || empty($the_new_price) ){
                    $other_error[] = [
                        'msg'   =>  '产品模板或产品规格的价格为0',
                        'tem_info'  =>  $the_tem_info,
                        'sku_info'  =>  $the_sku_info,
                        'old_price'  =>  $the_old_price,
                        'old_price_key' =>  $price_key_name,
                        'new_price'  =>  $the_new_price,
                        'new_price_key' =>  $new_price_key_name,
                    ];
                }

                //用该次循环的代理的下级产品价格减去这次的产品价格，再乘以产品数量即该产品的返利
                $the_price_bcsub = bcsub($the_old_price,$the_new_price,2);

                if( $the_price_bcsub > 0 ){
                    //$the_profit = $the_price_bcsub*$the_num + $the_profit;
                    $the_profit_new = bcmul($the_price_bcsub,$the_num,2);
                    $the_profit = bcadd($the_profit_new,$the_profit,2);
                }
            }
        ...
        if( !empty($recharge_error) ){
            setlog('订单返还充值失败：'.  print_r($recharge_error,1).',order_info:'.print_r($order_info,1),'order_return_recharge_error');
        }
        if( !empty($other_error) ){
            setlog('订单返还其它错误：'.  print_r($other_error,1).',order_info:'.print_r($order_info,1),'order_return_other_error');
        }
    ...
    /**
     * 返利审核时是否充值到用户可提现金额
     * //注意应该是先运行此方法再更改返利状态，可能会出现上级用户金额不足给下级充值
     * @param type $uid             返利人
     * @param type $money           返利金额
     * @param type $source_id       返利审核代理即返利支付者
     * @param type $note            备注
     * @return type
     */
    public function rebate_aduit_recharge($uid,$money,$source_id=0,$note=''){

        if( !$this->is_charge_money || !$this->is_rebate_recharge ){
            $return_result = [
                'code'  =>  1,
                'msg'   =>  '无须进行充值！',
            ];
        ...
        if( $source_id != 0 ){
            $charge_type = 'rebate_recharge';
            $charge_money_result = $this->charge_money($source_id,$money,$charge_type);
        ...
        $recharge_info['source_id'] = $source_id;
        $recharge_info['note'] = $note;

        $type = 'rebate_recharge';

        $result = $this->recharge($uid,$money,$type,$recharge_info);

        return $result;
    }//end func rebate_aduit_recharge
    ...
    public function month_count($data) {
        ...
        if (!$res) {
            setLog('虚拟币月统计失败:'.json_encode($data), 'money_count');
            return false;
        }
        return true;
    }
     ...
     //获取资金记录
    public function get_money_funds_log($page_info=array(),$condition=array()){
        ...
        if( $count > 0 ){
            ...
            if( !empty($page_info) ){
                ...
                $list = $money_funds_obj->where($condition)->order('id desc')->page($page_con)->select();
            }
            else{
                $list = $money_funds_obj->where($condition)->order('id desc')->select();
            }
    ...
    public function pay_callback_handle($result) {
        ...
        $money_apply_info =$money_apply_obj->where($where)->find();
        //保存支付交易号
        if ($money_apply_info) {
            $data = [
                'trade_num' => $result['transaction_id'],
            ];
            $money_apply_obj->where($where)->save($data);
        }else{
            $return_result=[
                'code'=>'-1',
                'msg' =>'在pay_callback_handle方法没有找到申请的充值信息'
            ];
            setLog('id'.$result['out_trade_no'].$return_result,'error-pay');
        }
        //触发审核方法
       $res= $this->apply_pass($money_apply_info['id'],1);
        if($res['code'] != 1){
            setLog('支付回调，审核充值申请失败，错误原因'.$res['msg'],'error-pay');
        }
