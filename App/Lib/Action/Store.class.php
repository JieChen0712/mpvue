...
//            }
    ...
    //获取联系信息
    public function get_contact($page_info=array(),$condition=array()){
        import('ORG.Util.Page');
        ...
        $count = $this->contact_model->where($condition)->count('id');
        ...
        if( $count > 0 ){
            if( !empty($page_info) ){
                ...
                $list = $this->contact_model->where($condition)->order('id desc')->page($page_con)->select();
            }
            else{
                $list = $this->contact_model->where($condition)->order('id desc')->select();
            }
